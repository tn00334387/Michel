...
By allowing the trained models to run on the actual distributed systems, which has not done by the previous work, the proposed framework enables the co-design of the distributed deep learning models and systems.
...
%Local execution module get the C/OpenCL model generated by the Device Module generator to do inference and optimize the code using accelerator for boosting inference performance. Local execution model also need to check the intermediate output during the inference to determine whether the result is confident or not, confident one would regard as the final classification and exit locally, or else would use Communication module sends to server for further computation. Since the end device need to know final classification, Communication module also receives the results from server. We designed intermediate output store in binary format to be smaller than the input, therefore the communication cost between end devices and server will be drastically reduced. The details of communicate will be discussed in the section 3-D.
 	...
 	\begin{figure}[tbh]
        \centering
        \includegraphics[width=0.96\columnwidth]{figure/dev_model_infer_code}
...
\emph{Scaling up} (i.e., vertical scaling) would be done by increasing the computing power and the memory space on the machine. %To \emph{scale up} (i.e., vertical scaling), increasing the computing power and the memory space on the machine would be suffice.
...
%\begin{table*}
%\centering
%\caption{The experimental result of feeding two different DNN models to our framework under the 1-to-1 model. T=0.4 is used in the experiments }
%\label{tab:experiment result}
%%\scriptsize{
%\begin{tabular}{|c|c||c|c|c|c|c|c|c|}
%\hline
%%\toprule
%\textbf{DNN Models}       & \textbf{Configurations}       & \textbf{Local Acc. (\%)} & \textbf{Time (s)} & \textbf{Threshold t} & \textbf{Exit (\%)} & \textbf{Server Usage} \\ \hline \hline
%\multirow{1}{*}{\textbf{\emph{Shallow}}}
%%                                                & \textbf{End device only}  &                    &                    &                      &                    &                       \\ \cline{2-7}
%                                                 & \textbf{Device\&Server} &                    &                    &                      &                    &                       \\ \cline{2-7}
%%                                                 & \textbf{Server only} &                    &                    &                      &                    &                       \\ \hline
%\hline \hline
%\multirow{2}{*}{\textbf{\emph{Deep}}}
%%                                                & \textbf{End device only}  &                    &                    &                      &                    &                       \\ \cline{2-7}
%                                                 & \textbf{Device\&Server} & 72.74\%            & 0.974              & 0.4                  & 2.38\%             & 293MiB                \\ \cline{2-7}
%                                                 & \textbf{Server only} & 65.76\%            & 0.118              & N/A                  & N/A                & 297MiB                \\ \hline
%\end{tabular}
%%} %end of scriptsize.
%\end{table*}
